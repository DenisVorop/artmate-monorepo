...
import { CatalogLandingDetails } from "@/features/catalog-landings-management";
import { routes } from "@/shared/constants";
import { Badge, Button } from "@/shared/ui";
import { AdminShell } from "@/widgets/admin-shell";

type CatalogLandingDetailPageProps = {
  readonly currentUser: AuthUser;
  readonly landingId: string;
};

export function CatalogLandingDetailPage({
  currentUser,
  landingId,
}: CatalogLandingDetailPageProps) {
  return (
      ...
      <section className="min-w-0 p-4 sm:p-6 lg:p-8">
        <header className="mb-6 flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
          <div className="min-w-0">
            ...
            <p className="mt-2 text-sm text-muted-foreground">{landingId}</p>
          </div>
        ...
        <CatalogLandingDetails landingId={landingId} />
      </section>
    </AdminShell>
  );
}
